...
class DeepQNetwork:
    ...
    def create_network(self):
        ...
        :return:
        """
        # state和action都是one_hot形式，即用一个1x6的0和1组成的序列表示
    ...
    def experience_replay(self):
        ...
        # state,action,reward,next_state,done是之前保存的记忆
        batch_state = None
        batch_action = None
        batch_reward = None
        ...
        # q_next：下一个状态的 Q 值。输入是多个state，所以输出是一个包含state和其所对应6个action的二维数组的三维数组
        ...
        for i in range(len(minibatch)):
            ...
            # 更新 Q 值。Q现实 = R + γ*Max[Q(s`, all actions)]，q_next[0][i]是state对应的6个action数组
